<div class="w-full max-w-6xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <div class="flex flex-col md:flex-row justify-between mb-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-6">
          <div>
            ...
            <p class="font-medium text-gray-800 dark:text-white"> {{ $order->created_at->format('d-m-Y') }} </p>
          </div>
          <div>
            <p class="text-sm text-gray-500 dark:text-gray-400">Total</p>
            <p class="font-medium text-blue-600 dark:text-blue-400"> {{ Number::currency($order->grand_total, 'INR') }} </p>
          </div>
          <div>
            <p class="text-sm text-gray-500 dark:text-gray-400">Paiement</p>
            <p class="font-medium text-gray-800 dark:text-white"> {{$order->payement_method == 'cod' ? 'Cash on Delivery' : 'Card' }} </p>
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          <div class="space-y-3">
            <div class="flex justify-between">
              ...
              <span class="font-medium text-gray-800 dark:text-white"> {{ Number::currency($order->grand_total, 'INR') }} </span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-600 dark:text-gray-300">Réduction</span>
              <span class="font-medium text-gray-800 dark:text-white"> {{ Number::currency(0, 'INR') }} </span>
            </div>
            <div class="flex justify-between">
            ...
            <div class="flex justify-between pt-3 border-t border-gray-200 dark:border-gray-700">
              ...
              <span class="text-lg font-semibold text-blue-600 dark:text-blue-400"> {{ Number::currency($order->grand_total, 'INR') }} </span>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="bg-blue-50 dark:bg-gray-700 rounded-lg p-4 transition-all duration-300 hover:scale-[1.02]">
            <div class="flex items-start space-x-3">
              ...
              <div>
                <h4 class="font-medium text-gray-800 dark:text-white"> {{ $order->address->full_name }} </h4>
                <p class="text-gray-600 dark:text-gray-300"> {{ $order->address->street_address }} </p>
                <p class="text-gray-600 dark:text-gray-300"> {{ $order->address->city }}, {{ $order->address->state }}, {{ $order->address->zip_code }} </p>
  ...
  <div class="flex flex-col sm:flex-row justify-center gap-4 px-4 mt-8 animate-slide-up">
    ...
    <a href="/my-orders" class="px-6 py-3 bg-blue-500 text-white rounded-lg font-medium text-center transition-all duration-300 hover:bg-blue-600 hover:shadow-md">
      Voir les détails
    </a>
  </div>
</div>